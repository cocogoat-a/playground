...
- If Google Sheets write capabilities are limited and structural delete operations are unavailable, prefer rewriting a compact cleaned table over making piecemeal single-cell edits across many rows and columns.
- If the browser session is view-only, fall back to Google MCP reads and writes immediately instead of spending time on UI-only cleanup attempts.
